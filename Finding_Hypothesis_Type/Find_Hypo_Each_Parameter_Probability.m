...
for c_param=1:all_params_number,
  ...
  for j_pr = 1:accuracy_level,
    ...
    for i_pr=start_of_cycle:length(powersArr),
      paramsArr_current=[paramsArr_optimal; i_pr; randomVector]
      
      powers_AverG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(1)).name]);
      powers_AverG = struct2cell(powers_AverG)(1,1){1,1}';
      powers_AverGA = powers_AverG;
      
      
      epowers_AverG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(2)).name]);
      epowers_AverG = struct2cell(epowers_AverG)(1,1){1,1}';
      epowers_AverGA = epowers_AverG;
      
      lnpowers_AverG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(3)).name]);
      lnpowers_AverG = struct2cell(lnpowers_AverG)(1,1){1,1}';
      lnpowers_AverGA = lnpowers_AverG;
      
      sqrtpowers_AverG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(4)).name]);
      sqrtpowers_AverG = struct2cell(sqrtpowers_AverG)(1,1){1,1}';
      sqrtpowers_AverGA = sqrtpowers_AverG;
      
      
      powers_AverXG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(5)).name]);
      powers_AverXG = struct2cell(powers_AverXG)(1,1){1,1}';
      powers_AverXGA = powers_AverXG;
      
      epowers_AverXG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(6)).name]);
      epowers_AverXG = struct2cell(epowers_AverXG)(1,1){1,1}';
      epowers_AverXGA = epowers_AverXG;
      
      lnpowers_AverXG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(7)).name]);
      lnpowers_AverXG = struct2cell(lnpowers_AverXG)(1,1){1,1}';
      lnpowers_AverXGA = lnpowers_AverXG;
      
      sqrtpowers_AverXG = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(8)).name]);
      sqrtpowers_AverXG = struct2cell(sqrtpowers_AverXG)(1,1){1,1}';
      sqrtpowers_AverXGA = sqrtpowers_AverXG;
      
      
      powers_PriceHome = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(9)).name]);
      powers_PriceHome = struct2cell(powers_PriceHome)(1,1){1,1}';
      powers_PriceAway= powers_PriceHome;
      
      epowers_PriceHome = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(10)).name]);
      epowers_PriceHome = struct2cell(epowers_PriceHome)(1,1){1,1}';
      epowers_PriceAway =epowers_PriceHome ;
      
      lnpowers_PriceHome = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(11)).name]);
      lnpowers_PriceHome = struct2cell(lnpowers_PriceHome)(1,1){1,1}';
      lnpowers_PriceAway = lnpowers_PriceHome;
      
      sqrtpowers_PriceHome = load([path '\Finding_Hypothesis_Type\powers\' powersArr(paramsArr_current(12)).name]);
      sqrtpowers_PriceHome = struct2cell(sqrtpowers_PriceHome)(1,1){1,1}';
     
      sqrtpowers_PriceAway = sqrtpowers_PriceHome;
      ...
      for cligas = 1:4,
        ...
        %Get prices
        Team_Price = fileread(['C:\Users\' PC '\Desktop\Dataset\' league '\' years '\priceOrigNames.txt']);
        ...
        sum=sum-betsNumber;
        sum_aver=sum_aver+sum;
        resRow=['Liga: ' ligasArr(cligas,:) '; AverG_powers: ' powersArr(paramsArr_current(1),1).name  '; AverXG_powers: ' powersArr(paramsArr_current(5),1).name 'Price_powers: ' powersArr( paramsArr_current(9),1).name '; AverG_epowers: ' powersArr(paramsArr_current(2),1).name  '; AverXG_epowers: ' powersArr(paramsArr_current(6),1).name '; Price_epowers: ' powersArr(paramsArr_current(10),1).name '; AverG_lnpowers: ' powersArr(paramsArr_current(3),1).name '; AverXG_lnpowers: ' powersArr(paramsArr_current(7),1).name '; Price_lnpowers: ' powersArr(paramsArr_current(11),1).name    'AverG_sqrtpowers' powersArr(paramsArr_current(4),1).name '; AverXG_sqrtpowers: ' powersArr(paramsArr_current(8),1).name '; Price_sqrtpowers: ' powersArr(paramsArr_current(12),1).name    '; SUM: ' num2str(sum)]

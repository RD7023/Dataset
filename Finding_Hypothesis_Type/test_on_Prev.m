...
AllHypothesis=fileread([path '\Finding_Hypothesis_Type\VB_Top\VB_' numberVB '.txt']);
...
for k =1:size(AllHypothesisArr,2),
  result = AllHypothesisArr{1,k};
  resultElements = strsplit(result,": ");
  fileNamesArr=[];
  for j = 3:size(resultElements,2),
    fileNamesArr=[fileNamesArr; strsplit(resultElements{1,j},';'){1,1}];
  endfor
  
  powers = load([path '\Finding_Hypothesis_Type\powers\' deblank(fileNamesArr(1,:))]);
  powers = struct2cell(powers)(1,1){1,1}';
  
  epowers = load([path '\Finding_Hypothesis_Type\epowers\' deblank(fileNamesArr(2,:))]);
  epowers = struct2cell(epowers)(1,1){1,1}';

  lnpowers = load([path '\Finding_Hypothesis_Type\lnpowers\' deblank(fileNamesArr(3,:))]);
  ...
  fid = fopen (['C:\Users\ZBook\Desktop\Dataset\Finding_Hypothesis_Type\Results_Prev.txt'], "a");
        fdisp (fid, resRow);
  fclose(fid);

...
for lni=3:length(lnpowersArr),
 lnpowers = load([path '\Finding_Hypothesis_Type\lnpowers\' lnpowersArr(lni,1).name]);
 lnpowers = struct2cell(lnpowers)(1,1){1,1}';

for ei=3:length(epowersArr),
 epowers = load([path '\Finding_Hypothesis_Type\epowers\' epowersArr(ei,1).name]);
 epowers = struct2cell(epowers)(1,1){1,1}';

for pwi=3:length(powersArr),
 powers = load([path '\Finding_Hypothesis_Type\powers\' powersArr(pwi,1).name]);
